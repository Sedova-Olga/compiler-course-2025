...
; CHECK-NEXT: entry:
; CHECK-NEXT: %sum = call i32 @add_i32(i32 %x, i32 %y)
...
; CHECK-NEXT: entry:
; CHECK-NEXT: %sum = call i64 @add_i64(i64 %x, i64 %y)
...
; CHECK-NEXT: entry:
; CHECK-NEXT: %sum = call float @add_float(float %x, float %y)
...
; CHECK: define float @bar_float(float %x, float %y)
; CHECK-NEXT: entry:
